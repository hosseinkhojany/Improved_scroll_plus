...
import 'package:untitled4/improvedScrolling/Throttler.dart';
import 'config.dart';
...
class ImprovedScrolling extends StatefulWidget {
  ...
  /// Theese will be make scrollable to scrolling forever
  final CustomAlwaysScrollingConfig? alwaysScrolling;
...
class _ImprovedScrollingState extends State<ImprovedScrolling> {
  ...
  var _alwaysScrollingActive = true;
          ...
          if (cursorIsWhereTheScrollStartedArea) {
            ...
            if (isVerticalAxis) {
              if (scrollingVelocity.dy > _mmbScrollLastVelocity.dy) {
                ...
                setMMBScrollCursorActivity(
                    MMBScrollCursorActivity.scrollingDown);
              }
            } else {
              if (scrollingVelocity.dx > _mmbScrollLastVelocity.dx) {
                setMMBScrollCursorActivity(
                    MMBScrollCursorActivity.scrollingLeft);
              } else {
                setMMBScrollCursorActivity(
  ...
  void scrollingForever() {
    if (widget.alwaysScrolling != null) {
      _timer ??= Timer.periodic(const Duration(milliseconds: 2000), (timer) {
        if (_alwaysScrollingActive && !_mmbScrollActive) {
